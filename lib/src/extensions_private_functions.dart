...
Future<void> _notify(ChannelType channel, ChannelEvent event) =>
...
Future<void> _notifyClosed(ChannelType channel) =>
    _notify(channel, ChannelEvent.closed);
Future<void> _notifyStarted(ChannelType channel) =>
    _notify(channel, ChannelEvent.started);
...
extension RaiseChannelFunctionExtension<Ret extends TypeSync>
...
extension ChannelTypeExtension on ChannelType {
  Future<bool> waitUntilStart(
      [Duration? timeout, bool pingFirst = true]) async {
    if (this == ThisChannel()._myChannelType) {
      return true;
    }
...
class _RaisePing extends ChannelFunction {
  ...
  _RaisePing.to(ChannelType toChannel)
      : super.to(
          toChannel: toChannel,
        );
...
class _RaiseChannelNotification extends ChannelFunction {
  ...
  TypeAsync callFunWithArgs() async {
    if (_notification.about == ThisChannel()._myChannelType) {
      return null;
    }
  ...
  _RaiseChannelNotification.fromJson(Map<String, dynamic> json)
      : _notification =
            ChannelNotification.fromJson(json[_notificationPropMarkupPropName]),
        super.fromJson(json);
}
